...
import { currencyFormatter0Digits, percentFormatter } from '../../shared/utils/NumberFormatter';
      ...
      <table cellSpacing={0} className={classes.table}>
        <thead>
          <tr>
            ...
            <th className={classes.th}>Inventory $ / Sq Ft</th>
            <th className={classes.th}>Days of Supply</th>
            <th className={classes.th}>GMROI</th>
            ...
            <tr key={row.id}>
              ...
              <td className={[classes.td, classes.cellCustomerPotential, classes.tdCustomerPotential].join(' ')}>
                <span className={classes.spanCustomerPotential} style={{ color: colors[idx].customerPotential}}>
                  {currencyFormatter0Digits(row.customerPotential)}
                </span>
                <div className={classes.divCustomerPotential}
                     style={{ width: getPercents(data.customerPotentialMaxValue, row.customerPotential) }}/>
              </td>
              <td className={classes.td} style={{ color: colors[idx].availability}}>
                {percentFormatter(row.availability)}
              </td>
              <td className={classes.td} style={{ color: colors[idx].inventory}}>
                {currencyFormatter0Digits(row.inventory)}
              </td>
              <td className={classes.td} style={{ color: colors[idx].daysOfSupply}}>
                {row.daysOfSupply}
              </td>
              <td className={classes.td} style={{ color: colors[idx].gmroi}}>
                {percentFormatter(row.gmroi)}
              </td>
